...
const instruction_t g_instr[256] = {
    ...
    {"LD BC, 0x", NULL, 2, NULL, NULL, ld_bc_nn},          // 0x01
    ...
    {"LD B, 0x", NULL, 1, NULL, ld_b_n, NULL},             // 0x06
    {"RLCA", NULL, 0, rlca, NULL, NULL},                   // 0x07
    {"LD (0x", "), SP", 2, NULL, NULL, ld_nnp_sp},         // 0x08
    ...
    {"LD C, 0x", NULL, 1, NULL, ld_c_n, NULL},             // 0x0e
    {"RRCA", NULL, 0, rrca, NULL, NULL},                   // 0x0f
    {"STOP", NULL, 0, stop, NULL, NULL},                   // 0x10
    {"LD DE, 0x", NULL, 2, NULL, NULL, ld_de_nn},          // 0x11
    ...
    {"LD D, 0x", NULL, 1, NULL, ld_d_n, NULL},             // 0x16
    {"RLA", NULL, 0, rla, NULL, NULL},                     // 0x17
    {"JR 0x", NULL, 1, NULL, jr_n, NULL},                  // 0x18
    ...
    {"LD E, 0x", NULL, 1, NULL, ld_e_n, NULL},             // 0x1e
    {"RRA", NULL, 0, rra, NULL, NULL},                     // 0x1f
    {"JR NZ, 0x", NULL, 1, NULL, jr_nz_n, NULL},           // 0x20
    {"LD HL, 0x", NULL, 2, NULL, NULL, ld_hl_nn},          // 0x21
    ...
    {"LD H, 0x", NULL, 1, NULL, ld_h_n, NULL},             // 0x26
    {"DAA", NULL, 0, daa, NULL, NULL},                     // 0x27
    {"JR Z, 0x", NULL, 1, NULL, jr_z_n, NULL},             // 0x28
    ...
    {"LD L, 0x", NULL, 1, NULL, ld_l_n, NULL},             // 0x2e
    {"CPL", NULL, 0, cpl, NULL, NULL},                     // 0x2f
    {"JR NC, 0x", NULL, 1, NULL, jr_nc_n, NULL},           // 0x30
    {"LD SP, 0x", NULL, 2, NULL, NULL, ld_sp_nn},          // 0x31
    ...
    {"LD (HL), 0x", NULL, 1, NULL, ld_hlp_n, NULL},        // 0x36
    {"SCF", NULL, 0, scf, NULL, NULL},                     // 0x37
    {"JR C, 0x", NULL, 1, NULL, jr_c_n, NULL},             // 0x38
    ...
    {"LD A, 0x", NULL, 1, NULL, ld_a_n, NULL},             // 0x3e
    ...
    {"JP NZ, 0x", NULL, 2, NULL, NULL, jp_nz_nn},          // 0xc2
    {"JP 0x", NULL, 2, NULL, NULL, jp_nn},                 // 0xc3
    {"CALL NZ, 0x", NULL, 2, NULL, NULL, call_nz_nn},      // 0xc4
    {"PUSH BC", NULL, 0, push_bc, NULL, NULL},             // 0xc5
    {"ADD A, 0x", NULL, 1, NULL, add_a_n, NULL},           // 0xc6
    {"RST 0x00", NULL, 0, rst_00, NULL, NULL},             // 0xc7
    {"RET Z", NULL, 0, ret_z, NULL, NULL},                 // 0xc8
    {"RET", NULL, 0, ret, NULL, NULL},                     // 0xc9
    {"JP Z, 0x", NULL, 2, NULL, NULL, jp_z_nn},            // 0xca
    {"CB ", NULL, 1, NULL, cb_n, NULL},                    // 0xcb
    {"CALL Z, 0x", NULL, 2, NULL, NULL, call_z_nn},        // 0xcc
    {"CALL 0x", NULL, 2, NULL, NULL, call_nn},             // 0xcd
    {"ADC 0x", NULL, 1, NULL, adc_n, NULL},                // 0xce
    {"RST 0x08", NULL, 0, rst_08, NULL, NULL},             // 0xcf
    {"RET NC", NULL, 0, ret_nc, NULL, NULL},               // 0xd0
    {"POP DE", NULL, 0, pop_de, NULL, NULL},               // 0xd1
    {"JP NC, 0x", NULL, 2, NULL, NULL, jp_nc_nn},          // 0xd2
    {"UNKNOWN", NULL, 0, undefined, NULL, NULL},           // 0xd3
    {"CALL NC, 0x", NULL, 2, NULL, NULL, call_nc_nn},      // 0xd4
    {"PUSH DE", NULL, 0, push_de, NULL, NULL},             // 0xd5
    {"SUB 0x", NULL, 1, NULL, sub_n, NULL},                // 0xd6
    {"RST 0x10", NULL, 0, rst_10, NULL, NULL},             // 0xd7
    {"RET C", NULL, 0, ret_c, NULL, NULL},                 // 0xd8
    {"RETI", NULL, 0, reti, NULL, NULL},                   // 0xd9
    {"JP C, 0x", NULL, 2, NULL, NULL, jp_c_nn},            // 0xda
    {"UNKNOWN", NULL, 0, undefined, NULL, NULL},           // 0xdb
    {"CALL C, 0x", NULL, 2, NULL, NULL, call_c_nn},        // 0xdc
    {"UNKNOWN", NULL, 0, undefined, NULL, NULL},           // 0xdd
    {"SBC 0x", NULL, 1, NULL, sbc_n, NULL},                // 0xde
    {"RST 0x18", NULL, 0, rst_18, NULL, NULL},             // 0xdf
    {"LDH (0xFF00 + 0x", "), A", 1, NULL, ldh_n_a, NULL},  // 0xe0
    {"POP HL", NULL, 0, pop_hl, NULL, NULL},               // 0xe1
    {"LD (0xFF00 + C), A", NULL, 0, ld_cp_a, NULL, NULL},  // 0xe2
    {"UNKNOWN", NULL, 0, undefined, NULL, NULL},           // 0xe3
    {"UNKNOWN", NULL, 0, undefined, NULL, NULL},           // 0xe4
    {"PUSH HL", NULL, 0, push_hl, NULL, NULL},             // 0xe5
    {"AND 0x", NULL, 1, NULL, and_n, NULL},                // 0xe6
    {"RST 0x20", NULL, 0, rst_20, NULL, NULL},             // 0xe7
    {"ADD SP,0x", NULL, 1, NULL, add_sp_n, NULL},          // 0xe8
    {"JP HL", NULL, 0, jp_hl, NULL, NULL},                 // 0xe9
    {"LD (0x", "), A", 2, NULL, NULL, ld_nnp_a},           // 0xea
    {"UNKNOWN", NULL, 0, undefined, NULL, NULL},           // 0xeb
    {"UNKNOWN", NULL, 0, undefined, NULL, NULL},           // 0xec
    {"UNKNOWN", NULL, 0, undefined, NULL, NULL},           // 0xed
    {"XOR 0x", NULL, 1, NULL, xor_n, NULL},                // 0xee
    {"RST 0x28", NULL, 0, rst_28, NULL, NULL},             // 0xef
    {"LDH A, (0xFF00 + 0x", ")", 1, NULL, ldh_a_n, NULL},  // 0xf0
    {"POP AF", NULL, 0, pop_af, NULL, NULL},               // 0xf1
    {"LD A, (0xFF00 + C)", NULL, 0, ld_a_cp, NULL, NULL},  // 0xf2
    {"DI", NULL, 0, di, NULL, NULL},                       // 0xf3
    {"UNKNOWN", NULL, 0, undefined, NULL, NULL},           // 0xf4
    {"PUSH AF", NULL, 0, push_af, NULL, NULL},             // 0xf5
    {"OR 0x", NULL, 1, NULL, or_n, NULL},                  // 0xf6
    {"RST 0x30", NULL, 0, rst_30, NULL, NULL},             // 0xf7
    {"LDHL, SP+0x", NULL, 1, NULL, ldhl_sp_n, NULL},       // 0xf8
    {"LD SP, HL", NULL, 0, ld_sp_hl, NULL, NULL},          // 0xf9
    {"LD A, (0x", ")", 2, NULL, NULL, ld_a_nnp},           // 0xfa
    {"EI", NULL, 0, ei, NULL, NULL},                       // 0xfb
    {"UNKNOWN", NULL, 0, undefined, NULL, NULL},           // 0xfc
    {"UNKNOWN", NULL, 0, undefined, NULL, NULL},           // 0xfd
    {"CP 0x", NULL, 1, NULL, cp_n, NULL},                  // 0xfe
    {"RST 0x38", NULL, 0, rst_38, NULL, NULL},             // 0xff
};
...
void cpu_debug_instr(char *str, size_t size)
{
    ...
    if (oper_length == 0) {
        ...
        snprintf(str, size, "Next instr: 0x%02x: %s%02x%s", opcode, asm1,
                 operand, asm2);
    } else {
        uint16_t operand = mmu_read_word(pctmp);
        snprintf(str, size, "Next instr: 0x%02x: %s%04x%s", opcode, asm1,
                 operand, asm2);
    }
}
